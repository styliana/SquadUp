    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex gap-6 border-b border-white/10 mb-8">
        <button 
          ...
          className={`pb-4 px-2 text-lg font-medium transition-all relative ${activeTab === 'published' ? 'text-white' : 'text-textMuted hover:text-white'}`}
        >
          Published Projects 
          <span className="ml-2 text-xs bg-white/10 px-2 py-0.5 rounded-full text-textMuted">{createdProjects.length}</span>
        ...
        <button 
          ...
          className={`pb-4 px-2 text-lg font-medium transition-all relative ${activeTab === 'applied' ? 'text-white' : 'text-textMuted hover:text-white'}`}
        >
          Applications Sent
          <span className="ml-2 text-xs bg-white/10 px-2 py-0.5 rounded-full text-textMuted">{appliedProjects.length}</span>
                ...
                <div className="p-6 border-b border-white/5 bg-white/[0.02] flex flex-col md:flex-row justify-between items-start gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h2 className="text-2xl font-bold text-white hover:text-primary transition-colors cursor-pointer" onClick={() => navigate(`/projects/${project.id}`)}>
                        {project.title}
                      </h2>
                      ...
                      {project.applications.map(app => (
                        <div key={app.id} className="bg-surface border border-white/5 rounded-xl p-4 flex flex-col md:flex-row items-center gap-4 hover:border-white/10 transition-colors">
                          ...
                          <div className="flex items-center gap-4 flex-grow w-full md:w-auto">
                            ...
                            <div className="flex-grow min-w-0">
                              <div className="flex items-baseline gap-2">
                                <span className="font-bold text-white text-base hover:underline cursor-pointer" onClick={() => navigate(`/profile/${app.profiles?.id}`)}>
                                  {app.profiles?.full_name || 'Anonymous'}
                                </span>
                                <span className="text-xs text-textMuted truncate">{app.profiles?.university}</span>
                              </div>
                              <div className="mt-1.5 text-sm text-gray-400 bg-black/20 p-2 rounded-lg border-l-2 border-primary italic">
                                "{app.message}"
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-3 shrink-0 mt-3 md:mt-0 w-full md:w-auto justify-end">
                            {app.status === 'pending' ? (
                              <>
                                <button onClick={() => handleStatusChange(app.id, project.id, 'accepted')} className="flex items-center gap-1 px-3 py-2 bg-green-500/10 text-green-400 rounded-lg border border-green-500/20 hover:bg-green-500/20 transition-colors text-sm font-medium">
                                  <Check size={16} /> Accept
                                </button>
                                <button onClick={() => handleStatusChange(app.id, project.id, 'rejected')} className="flex items-center gap-1 px-3 py-2 bg-red-500/10 text-red-400 rounded-lg border border-red-500/20 hover:bg-red-500/20 transition-colors text-sm font-medium">
                                  <X size={16} /> Reject
                                </button>
                              </>
                            ) : (
      ...
      {activeTab === 'applied' && (
        <div className="grid grid-cols-1 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {appliedProjects.length === 0 ? (
             <div className="text-center py-20 text-textMuted border border-dashed border-white/10 rounded-3xl">
               You haven't applied to any projects yet. Go find some!
             </div>
          ) : (
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-6 pl-2">
                  ...
                  <div className="flex-grow">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-xl font-bold text-white group-hover:text-primary transition-colors">
                        {app.projects?.title || 'Project Removed'}
                      </h3>
                      <StatusBadge status={app.status} />
                    </div>
                    
                    <div className="flex flex-wrap gap-4 text-sm text-textMuted mb-4">
                      <span className="bg-white/5 px-2.5 py-0.5 rounded-md text-gray-300 border border-white/5">
                        {app.projects?.type}
                      </span>
                  ...
                  <div className="flex flex-col items-end gap-3 min-w-[140px]">
                    <Link 
                      ...
                      className="w-full py-2 px-4 rounded-xl border border-white/10 text-white text-sm font-medium hover:bg-white/5 hover:border-white/20 transition-all flex items-center justify-center gap-2"
                    >
                      <Eye size={16} />
                      View Project
                    </Link>
