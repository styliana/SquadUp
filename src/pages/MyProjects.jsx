...
const MyProjects = () => {
  ...
  const handleDeleteProject = async (projectId) => {
    ...
    try {
      setLoading(true); // Opcjonalnie: pokaż loader

      // 1. KROK KLUCZOWY: Najpierw usuń zgłoszenia powiązane z projektem
      // Dzięki nowej polityce SQL, teraz to zadziała!
      ...
      if (appError) {
        console.error("Błąd usuwania aplikacji:", appError);
        throw appError;
      }

      // 2. Dopiero teraz usuń sam projekt
      ...
      // Jeśli błąd, odśwież dane, żeby user widział stan faktyczny
      fetchData(); 
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="flex border-b border-white/10 mb-8">
        <button 
          ...
          className={`pb-4 px-6 text-lg font-medium transition-all relative ${activeTab === 'published' ? 'text-primary' : 'text-textMuted hover:text-white'}`}
        >
        ...
        <button 
          ...
          className={`pb-4 px-6 text-lg font-medium transition-all relative ${activeTab === 'applied' ? 'text-primary' : 'text-textMuted hover:text-white'}`}
        >
                      ...
                      {project.applications.map(app => (
                        <div key={app.id} className="bg-background border border-white/10 rounded-xl p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            {app.status === 'pending' ? (
                              <>
                                <button onClick={() => handleStatusChange(app.id, 'accepted')} className="p-2 bg-green-500/10 text-green-400 rounded-lg hover:bg-green-500/20 border border-green-500/20" title="Accept"><Check size={18} /></button>
                                <button onClick={() => handleStatusChange(app.id, 'rejected')} className="p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 border border-red-500/20" title="Reject"><X size={18} /></button>
                              </>
                            ) : (
                            ...
                            <Link to="/chat" className="p-2 text-textMuted hover:text-white" title="Chat"><MessageCircle size={18} /></Link>
                            <Link to={`/profile`} className="p-2 text-textMuted hover:text-white" title="Profile"><User size={18} /></Link>
                          </div>
                        </div>
                      ))}
                    </div>
