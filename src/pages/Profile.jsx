...
import { toast } from 'sonner'; // <--- Importujemy funkcję toast
...
const Profile = () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          skills: profile.skills, // Teraz to zadziała, bo dodaliśmy kolumnę w SQL
          updated_at: new Date(),
        })
        .eq('id', user.id);
      ...
      setIsEditing(false);
      // TU BYŁ ALERT -> TERAZ JEST TOAST
      toast.success('Profile updated successfully! 🔥');
      
    } catch (error) {
      console.error(error);
      // TU TEŻ JEST TOAST BŁĘDU
  ...
  // (Poniżej wklejam resztę, żebyś miał pewność, że wszystko pasuje)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-white">Your <span className="text-primary">Profile</span></h1>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-grow w-full space-y-5">
                ...
                <div>
                  ...
                  {isEditing ? (
                    <input type="text" value={profile.university || ''} onChange={e => setProfile({...profile, university: e.target.value})} className="w-full bg-background border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-primary" placeholder="University..." />
            ...
            {isEditing ? (
              ...
              <div className="flex flex-wrap gap-2">
                {profile.skills?.map(skill => (
                  <span key={skill} className="px-3 py-1.5 rounded-lg bg-background border border-white/10 text-gray-300 text-sm">{skill}</span>
                ))}
        ...
        <div className="space-y-6">
          <div className="bg-surface border border-white/5 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                {isEditing ? (
                   <input type="text" value={profile.website || ''} onChange={e => setProfile({...profile, website: e.target.value})} className="flex-grow bg-background border border-white/10 rounded px-2 py-1 text-white text-sm" placeholder="Website URL" />
          ...
          <div className="bg-surface border border-white/5 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center"><span className="text-textMuted">Projects</span><span className="font-bold text-white text-lg">{profile.stats_projects}</span></div>
              <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-primary w-1/4"></div></div>
            </div>
          </div>
        </div>
      </div>
